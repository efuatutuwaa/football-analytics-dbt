...
# ── Incremental check ─────────────────────────────────────
def get_last_ingested_at(cursor, endpoint: str, team_id: int = None):
...
def load_to_databricks(
    ...
    with sql.connect(
        ...
        with connection.cursor() as cursor:
            ...
            if last_ingested_at:
                print(f"  Team {team_id} coaches already ingested — skipping")
                return
...
def main():
    ...
    try:
        for team_id in team_ids:
            ...
            if not records:
                print(f"  No coaches found for team {team_id} — skipping")
                continue
